 ...
 *s++ = 4  % 10 + '0'; // s = {'4'}       <- n now gets assigned 48 automatically
 *s = '\0';            // s = {'4', '\0'}
 *s++ = 48 % 10 + '0'; // s = {'4', '8'}  <- we overwrite last NULL
 *s = '\0';            // s = {'4', '8', '\0'}
...
void itoa(int n, char *s)
{
    ...
    {                       /* if negative convert n to a positive int */
        *s++ = '-';         /* but add the '-' char at the same time!  */
        n = -n; 
    }
    ...
    *s++ = n % 10 + '0';    /* write the number in reverse!  */
    *s = '\0';
}
